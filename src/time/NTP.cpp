...
boolean NTP::synchronize(String server, int offset){
  WiFiUDP udp;
  udp.begin(123);
  ...
  while (millis () - beginWait < 1500) {
      if (udp.available() >= 48) {
        ...
        if(std::localtime(&t)){
            ...
            Logger::getInstance()->write("Current time set to "+String(std::asctime (&time)));
            RTC::setTime(time);
            udp.stop ();
            return true;
        }
...
void NTP::writeNTPPacket(WiFiUDP &udp, String server){
...
time_t NTP::readNTPPacket(WiFiUDP &udp, int offset){
